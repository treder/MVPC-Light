...
% .narrow_weight    [matrix] [n_narrow, n_channel] matrix of weights that
%                           show how a source projects into sensor space.
%                           If not provided, random weights are created and
%                           set to norm 1 each
% .broad_weight     [matrix] [n_broad, n_channel] matrix for the broadband
%                           sources.
...
assert(isempty(cfg.narrow_weight) || all(size(cfg.narrow_weight) == [cfg.n_narrow, cfg.n_channel]), 'narrow_weight must be empty or a [n_signal, n_channel] matrix')
assert(isempty(cfg.broad_weight) || all(size(cfg.broad_weight) == [n_broad, cfg.n_channel]), 'broad_weight must be empty or a [n_noise, n_channel] matrix')
...
if isempty(cfg.narrow_weight)
    cfg.narrow_weight = randn(cfg.n_narrow, cfg.n_channel);
...
if isempty(cfg.broad_weight)
    cfg.broad_weight = randn(cfg.n_broad, cfg.n_channel);
